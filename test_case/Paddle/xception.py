...
class SeparableConv(paddle.nn.Layer):
...
class EntryFlowBottleneckBlock(paddle.nn.Layer):
    def __init__(self,
                 input_channels,
                 output_channels,
                 stride=2,
    ...
    def forward(self, inputs):
        ...
        if self.relu_first:
            conv0 = paddle.nn.functional.relu(conv0)
        conv1 = self._conv1(conv0)
        conv2 = paddle.nn.functional.relu(conv1)
        conv2 = self._conv2(conv2)
        pool = self._pool(conv2)
        return paddle.add(x=short, y=pool)


class EntryFlow(paddle.nn.Layer):
...
class MiddleFlowBottleneckBlock(paddle.nn.Layer):
    ...
    def forward(self, inputs):
        conv0 = paddle.nn.functional.relu(inputs)
        conv0 = self._conv_0(conv0)
        conv1 = paddle.nn.functional.relu(conv0)
...
class MiddleFlow(paddle.nn.Layer):
...
class ExitFlowBottleneckBlock(paddle.nn.Layer):
...
class ExitFlow(paddle.nn.Layer):
