...
class InvertedResidual(paddle.nn.Module):
    # Implemented as described at section 5 of MobileNetV3 paper
    def __init__(
        self,
        cnf: InvertedResidualConfig,
        norm_layer: Callable[..., paddle.nn.Module],
        se_layer: Callable[..., paddle.nn.Module] = partial(SElayer, scale_activation=paddle.nn.Hardsigmoid),
    ):
        super().__init__()
...
class MobileNetV3(paddle.nn.Module):
    def __init__(
        self,
        inverted_residual_setting: List[InvertedResidualConfig],
        last_channel: int,
        num_classes: int = 1000,
        block: Optional[Callable[..., paddle.nn.Module]] = None,
        norm_layer: Optional[Callable[..., paddle.nn.Module]] = None,
        dropout: float = 0.2,
        **kwargs: Any,
    ) -> None:
        """
        ...
        if norm_layer is None:
            norm_layer = partial(paddle.nn.BatchNorm2d, eps=0.001, momentum=0.01)
        ...
        self.classifier = paddle.nn.Sequential(
            paddle.nn.Linear(lastconv_output_channels, last_channel),
            paddle.nn.Hardswish(inplace=True),
            paddle.nn.Dropout(p=dropout, inplace=True),
            paddle.nn.Linear(last_channel, num_classes),
        )

        for m in self.modules():
            if isinstance(m, paddle.nn.Conv2d):
                paddle.nn.init.kaiming_normal_(m.weight, mode="fan_out")
                if m.bias is not None:
                    paddle.nn.init.zeros_(m.bias)
            elif isinstance(m, (paddle.nn.BatchNorm2d, paddle.nn.GroupNorm)):
                paddle.nn.init.ones_(m.weight)
                paddle.nn.init.zeros_(m.bias)
            elif isinstance(m, paddle.nn.Linear):
                paddle.nn.init.normal_(m.weight, 0, 0.01)
                paddle.nn.init.zeros_(m.bias)
