...
class InvertedResidual(torch.nn.Module):
    def __init__(self, inp, oup, stride, benchmodel):
        ...
        if self.benchmodel == 1:
            #assert inp == oup_inc
        	self.banch2 = torch.nn.Sequential(
                # pw
            ...
            self.banch1 = torch.nn.Sequential(
                # dw
...
class ShuffleNetV2(torch.nn.Module):
	...
	self.globalpool = torch.nn.Sequential(torch.nn.AvgPool2d(int(input_size/32)))              
    
	# building classifier
	self.classifier = torch.nn.Sequential(torch.nn.Linear(self.stage_out_channels[-1], n_class))
