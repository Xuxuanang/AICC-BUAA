...
class MBConv(torch.nn.Module):
    def __init__(
        ...
        se_layer: Callable[..., torch.nn.Module] = SqueezeExcitation,
    ) -> None:
        super().__init__()
        ...
        layers.append(se_layer(expanded_channels, squeeze_channels, activation=partial(torch.nn.SiLU, inplace=True)))
...
class EfficientNet(torch.nn.Module):
    def __init__(
        ...
        layers.append(
            Conv2dNormActivation(
                3, firstconv_output_channels, kernel_size=3, stride=2, norm_layer=norm_layer, activation_layer=torch.nn.SiLU
            )
        )
        ...
        self.classifier = torch.nn.Sequential(
            ...
            torch.nn.Linear(lastconv_output_channels, num_classes),
        )
