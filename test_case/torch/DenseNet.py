...
class _DenseLayer(torch.nn.Module):
    def __init__(
        ...
        super().__init__()
        self.norm1 = torch.nn.BatchNorm2d(num_input_features)
        self.relu1 = torch.nn.ReLU(inplace=True)
        self.conv1 = torch.nn.Conv2d(num_input_features, bn_size * growth_rate, kernel_size=1, stride=1, bias=False)

        self.norm2 = torch.nn.BatchNorm2d(bn_size * growth_rate)
...
class _Transition(torch.nn.Sequential):
    def __init__(self, num_input_features: int, num_output_features: int) -> None:
        super().__init__()
        self.norm = torch.nn.BatchNorm2d(num_input_features)
...
def _load_state_dict(model: torch.nn.Module, weights: WeightsEnum, progress: bool) -> None:
