...
class InvertedResidual(paddle.nn.Module):    # 当前算子不支持转换

    def __init__(self, cnf: InvertedResidualConfig, norm_layer: Callable[..., paddle.nn.Module], se_layer: Callable[..., paddle.nn.Module]=partial(SElayer, scale_activation=paddle.nn.Hardsigmoid)):    # 当前算子不支持转换
        super().__init__()
        if not 1 <= cnf.stride <= 2:
            raise ValueError('illegal stride value')
        self.use_res_connect = cnf.stride == 1 and cnf.input_channels == cnf.out_channels
        layers: List[paddle.nn.Module] = []    # 当前算子不支持转换
        activation_layer = paddle.nn.Hardswish if cnf.use_hs else paddle.nn.ReLU    # 当前算子不支持转换
        ...
        self.block = paddle.nn.Sequential(*layers)    # 当前算子不支持转换
...
class MobileNetV3(paddle.nn.Module):    # 当前算子不支持转换

    def __init__(self, inverted_residual_setting: List[InvertedResidualConfig], last_channel: int, num_classes: int=1000, block: Optional[Callable[..., paddle.nn.Module]]=None, norm_layer: Optional[Callable[..., paddle.nn.Module]]=None, dropout: float=0.2, **kwargs: Any) -> None:    # 当前算子不支持转换
        """
        ...
        Args:
            ...
            block (Optional[Callable[..., paddle.nn.Module]]): Module specifying inverted residual building block for mobilenet
            norm_layer (Optional[Callable[..., paddle.nn.Module]]): Module specifying the normalization layer to use
        ...
        if norm_layer is None:
            norm_layer = partial(paddle.nn.BatchNorm2d, eps=0.001, momentum=0.01)    # 当前算子不支持转换
        layers: List[paddle.nn.Module] = []    # 当前算子不支持转换
        firstconv_output_channels = inverted_residual_setting[0].input_channels
        layers.append(Conv2dNormActivation(3, firstconv_output_channels, kernel_size=3, stride=2, norm_layer=norm_layer, activation_layer=paddle.nn.Hardswish))    # 当前算子不支持转换
        for cnf in inverted_residual_setting:
            layers.append(block(cnf, norm_layer))
        lastconv_input_channels = inverted_residual_setting[-1].out_channels
        lastconv_output_channels = 6 * lastconv_input_channels
        layers.append(Conv2dNormActivation(lastconv_input_channels, lastconv_output_channels, kernel_size=1, norm_layer=norm_layer, activation_layer=paddle.nn.Hardswish))    # 当前算子不支持转换
        self.features = paddle.nn.Sequential(*layers)    # 当前算子不支持转换
        self.avgpool = paddle.nn.AdaptiveAvgPool2d(1)    # 当前算子不支持转换
        self.classifier = paddle.nn.Sequential(paddle.nn.Linear(lastconv_output_channels, last_channel), paddle.nn.Hardswish(inplace=True), paddle.nn.Dropout(p=dropout, inplace=True), paddle.nn.Linear(last_channel, num_classes))    # 当前算子不支持转换
        for m in self.modules():
            if isinstance(m, paddle.nn.Conv2d):    # 当前算子不支持转换
                paddle.nn.init.kaiming_normal_(m.weight, mode='fan_out')    # 当前算子不支持转换
                if m.bias is not None:
                    paddle.nn.init.zeros_(m.bias)    # 当前算子不支持转换
            elif isinstance(m, (paddle.nn.BatchNorm2d, paddle.nn.GroupNorm)):    # 当前算子不支持转换
                paddle.nn.init.ones_(m.weight)    # 当前算子不支持转换
                paddle.nn.init.zeros_(m.bias)    # 当前算子不支持转换
            elif isinstance(m, paddle.nn.Linear):    # 当前算子不支持转换
                paddle.nn.init.normal_(m.weight, 0, 0.01)    # 当前算子不支持转换
                paddle.nn.init.zeros_(m.bias)    # 当前算子不支持转换
