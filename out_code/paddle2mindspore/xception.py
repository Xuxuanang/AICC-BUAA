...
MODEL_URLS = {'Xception41': 'https://paddle-imagenet-models-name.bj.bcebos.com/dygraph/Xception41_pretrained.pdparams', 'Xception65': 'https://paddle-imagenet-models-name.bj.bcebos.com/dygraph/Xception65_pretrained.pdparams', 'Xception71': 'https://paddle-imagenet-models-name.bj.bcebos.com/dygraph/Xception71_pretrained.pdparams'}
...
class ConvBNLayer(paddle.nn.Layer):    # 当前算子不支持转换
    ...
    def __init__(self, num_channels, num_filters, filter_size, stride=1, groups=1, act=None, name=None):
        ...
        bn_name = 'bn_' + name
        self._batch_norm = BatchNorm(num_filters, act=act, param_attr=ParamAttr(name=bn_name + '_scale'), bias_attr=ParamAttr(name=bn_name + '_offset'), moving_mean_name=bn_name + '_mean', moving_variance_name=bn_name + '_variance')
...
class SeparableConv(paddle.nn.Layer):    # 当前算子不支持转换
    ...
    def __init__(self, input_channels, output_channels, stride=1, name=None):
        ...
        self._depthwise_conv = ConvBNLayer(output_channels, output_channels, 3, stride=stride, groups=output_channels, name=name + '_dw')
...
class EntryFlowBottleneckBlock(paddle.nn.Layer):    # 当前算子不支持转换

    def __init__(self, input_channels, output_channels, stride=2, name=None, relu_first=False):
        super(EntryFlowBottleneckBlock, self).__init__()
        self.relu_first = relu_first
        self._short = Conv2D(in_channels=input_channels, out_channels=output_channels, kernel_size=1, stride=stride, padding=0, weight_attr=ParamAttr(name + '_branch1_weights'), bias_attr=False)
    ...
    def forward(self, inputs):
        ...
        if self.relu_first:
            conv0 = mindspore.mint.nn.functional.relu(conv0)
        conv1 = self._conv1(conv0)
        conv2 = mindspore.mint.nn.functional.relu(conv1)
        conv2 = self._conv2(conv2)
        pool = self._pool(conv2)
        return mindspore.mint.add(x=short, y=pool)

class EntryFlow(paddle.nn.Layer):    # 当前算子不支持转换
...
class MiddleFlowBottleneckBlock(paddle.nn.Layer):    # 当前算子不支持转换
    ...
    def forward(self, inputs):
        conv0 = mindspore.mint.nn.functional.relu(inputs)
        conv0 = self._conv_0(conv0)
        conv1 = mindspore.mint.nn.functional.relu(conv0)
        conv1 = self._conv_1(conv1)
        conv2 = mindspore.mint.nn.functional.relu(conv1)
        conv2 = self._conv_2(conv2)
        return mindspore.mint.add(x=inputs, y=conv2)

class MiddleFlow(paddle.nn.Layer):    # 当前算子不支持转换
...
class ExitFlowBottleneckBlock(paddle.nn.Layer):    # 当前算子不支持转换

    def __init__(self, input_channels, output_channels1, output_channels2, name):
        super(ExitFlowBottleneckBlock, self).__init__()
        self._short = Conv2D(in_channels=input_channels, out_channels=output_channels2, kernel_size=1, stride=2, padding=0, weight_attr=ParamAttr(name + '_branch1_weights'), bias_attr=False)
...
class ExitFlow(paddle.nn.Layer):    # 当前算子不支持转换
    ...
    def __init__(self, class_num):
        ...
        self._pool = mindspore.mint.nn.AdaptiveAvgPool2d()
        stdv = 1.0 / math.sqrt(2048 * 1.0)
        self._out = mindspore.mint.nn.Linear(class_num)

    def forward(self, inputs):
        conv0 = self._conv_0(inputs)
        conv1 = self._conv_1(conv0)
        conv1 = mindspore.mint.nn.functional.relu(conv1)
        conv2 = self._conv_2(conv1)
        conv2 = mindspore.mint.nn.functional.relu(conv2)
        pool = self._pool(conv2)
        pool = mindspore.mint.ravel(pool, start_axis=1, stop_axis=-1)
        out = self._out(pool)
        return out

class Xception(paddle.nn.Layer):    # 当前算子不支持转换

    def __init__(self, entry_flow_block_num=3, middle_flow_block_num=8, class_num=1000):
