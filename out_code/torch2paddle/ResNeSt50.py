...
class GlobalAvgPool2d(paddle.nn.Layer):
    ...
    def forward(self, inputs):
        return torch.nn.functional.adaptive_avg_pool2d(inputs, 1).view(inputs.size(0), -1)    # 当前算子不支持转换
...
class Bottleneck(paddle.nn.Layer):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, radix=1, cardinality=1, bottleneck_width=64, avd=False, avd_first=False, dilation=1, is_first=False, rectified_conv=False, rectify_avg=False, norm_layer=None, dropblock_prob=0.0, last_gamma=False):
        ...
        self.conv1 = paddle.nn.Conv2D(inplanes, False, stride=group_width, kernel_size=1)
        ...
        if self.avd:
            self.avd_layer = paddle.nn.AvgPool2D(stride, 3, 1)
            stride = 1
        ...
        if radix >= 1:
            ...
            self.conv2 = paddle.nn.Conv2D(group_width, False, cardinality, padding=dilation, stride=group_width, kernel_size=3, dilation=dilation)
            self.bn2 = norm_layer(group_width)
        self.conv3 = paddle.nn.Conv2D(group_width, False, stride=planes * 4, kernel_size=1)
        self.bn3 = norm_layer(planes * 4)
        if last_gamma:
            from torch.nn.init import zeros_    # 当前算子不支持转换
...
class ResNet(paddle.nn.Layer):
    ...
    norm_layer : object
        Normalization layer used in backbone network (default: :class:`mxnet.gluon.torch.nn.BatchNorm`;
    ...
    def __init__(self, block, layers, radix=1, groups=1, bottleneck_width=64, num_classes=1000, dilated=False, dilation=1, deep_stem=False, stem_width=64, avg_down=False, rectified_conv=False, rectify_avg=False, avd=False, avd_first=False, final_drop=0.0, dropblock_prob=0, last_gamma=False, norm_layer=torch.nn.BatchNorm2d):    # 当前算子不支持转换
        ...
        if deep_stem:
            self.conv1 = paddle.nn.Sequential(conv_layer(3, stem_width, kernel_size=3, stride=2, padding=1, bias=False, **conv_kwargs), norm_layer(stem_width), paddle.nn.ReLU(), conv_layer(stem_width, stem_width, kernel_size=3, stride=1, padding=1, bias=False, **conv_kwargs), norm_layer(stem_width), paddle.nn.ReLU(), conv_layer(stem_width, stem_width * 2, kernel_size=3, stride=1, padding=1, bias=False, **conv_kwargs))
        else:
            self.conv1 = conv_layer(3, 64, kernel_size=7, stride=2, padding=3, bias=False, **conv_kwargs)
        self.bn1 = norm_layer(self.inplanes)
        self.relu = paddle.nn.ReLU()
        self.maxpool = paddle.nn.MaxPool2D(2, kernel_size=3, padding=1)
        ...
        self.drop = paddle.nn.Dropout() if final_drop > 0.0 else None
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilation=1, norm_layer=None, dropblock_prob=0.0, is_first=True):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            ...
            if self.avg_down:
                if dilation == 1:
                    down_layers.append(torch.nn.AvgPool2d(kernel_size=stride, stride=stride, ceil_mode=True, count_include_pad=False))    # 当前算子不支持转换
                else:
                    down_layers.append(torch.nn.AvgPool2d(kernel_size=1, stride=1, ceil_mode=True, count_include_pad=False))    # 当前算子不支持转换
                down_layers.append(torch.nn.Conv2d(self.inplanes, planes * block.expansion, kernel_size=1, stride=1, bias=False))    # 当前算子不支持转换
            else:
                down_layers.append(torch.nn.Conv2d(self.inplanes, planes * block.expansion, kernel_size=1, stride=stride, bias=False))    # 当前算子不支持转换
            down_layers.append(norm_layer(planes * block.expansion))
            downsample = paddle.nn.Sequential(*down_layers)
        layers = []
...
def resnet50(pretrained=False, root='~/.encoding/models', **kwargs):
    ...
    if pretrained:
        model.load_state_dict(torch.hub.load_state_dict_from_url(resnest_model_urls['resnet50'], progress=True, check_hash=True))    # 当前算子不支持转换
    return model
...
def resnet101(pretrained=False, root='~/.encoding/models', **kwargs):
    ...
    if pretrained:
        model.load_state_dict(torch.hub.load_state_dict_from_url(resnest_model_urls['resnet101'], progress=True, check_hash=True))    # 当前算子不支持转换
    return model
...
def resnet152(pretrained=False, root='~/.encoding/models', **kwargs):
    ...
    if pretrained:
        model.load_state_dict(torch.hub.load_state_dict_from_url(resnest_model_urls['resnet152'], progress=True, check_hash=True))    # 当前算子不支持转换
    return model