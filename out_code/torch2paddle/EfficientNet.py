...
class MBConvConfig(_MBConvConfig):
    ...
    def __init__(self, expand_ratio: float, kernel: int, stride: int, input_channels: int, out_channels: int, num_layers: int, width_mult: float=1.0, depth_mult: float=1.0, block: Optional[Callable[..., torch.nn.Module]]=None) -> None:    # 当前算子不支持转换
...
class FusedMBConvConfig(_MBConvConfig):
    ...
    def __init__(self, expand_ratio: float, kernel: int, stride: int, input_channels: int, out_channels: int, num_layers: int, block: Optional[Callable[..., torch.nn.Module]]=None) -> None:    # 当前算子不支持转换
        if block is None:
            block = FusedMBConv
        super().__init__(expand_ratio, kernel, stride, input_channels, out_channels, num_layers, block)
...
class MBConv(paddle.nn.Layer):
    ...
    def __init__(self, cnf: MBConvConfig, stochastic_depth_prob: float, norm_layer: Callable[..., torch.nn.Module], se_layer: Callable[..., torch.nn.Module]=SqueezeExcitation) -> None:    # 当前算子不支持转换
        super().__init__()
        if not 1 <= cnf.stride <= 2:
            raise ValueError('illegal stride value')
        self.use_res_connect = cnf.stride == 1 and cnf.input_channels == cnf.out_channels
        layers: List[paddle.nn.Layer] = []
        activation_layer = paddle.nn.Silu
        ...
        layers.append(se_layer(expanded_channels, squeeze_channels, activation=partial(torch.nn.SiLU, inplace=True)))    # 当前算子不支持转换
...
class FusedMBConv(paddle.nn.Layer):

    def __init__(self, cnf: FusedMBConvConfig, stochastic_depth_prob: float, norm_layer: Callable[..., torch.nn.Module]) -> None:    # 当前算子不支持转换
        super().__init__()
        if not 1 <= cnf.stride <= 2:
            raise ValueError('illegal stride value')
        self.use_res_connect = cnf.stride == 1 and cnf.input_channels == cnf.out_channels
...
class EfficientNet(paddle.nn.Layer):

    def __init__(self, inverted_residual_setting: Sequence[Union[MBConvConfig, FusedMBConvConfig]], dropout: float, stochastic_depth_prob: float=0.2, num_classes: int=1000, norm_layer: Optional[Callable[..., torch.nn.Module]]=None, last_channel: Optional[int]=None) -> None:    # 当前算子不支持转换
        """
        ...
        Args:
            ...
            norm_layer (Optional[Callable[..., torch.nn.Module]]): Module specifying the normalization layer to use
        ...
        layers.append(Conv2dNormActivation(3, firstconv_output_channels, kernel_size=3, stride=2, norm_layer=norm_layer, activation_layer=torch.nn.SiLU))    # 当前算子不支持转换
        ...
        for cnf in inverted_residual_setting:
            ...
            for _ in range(cnf.num_layers):
                ...
                stage_block_id += 1
            layers.append(torch.nn.Sequential(*stage))    # 当前算子不支持转换
        lastconv_input_channels = inverted_residual_setting[-1].out_channels
        lastconv_output_channels = last_channel if last_channel is not None else 4 * lastconv_input_channels
        layers.append(Conv2dNormActivation(lastconv_input_channels, lastconv_output_channels, kernel_size=1, norm_layer=norm_layer, activation_layer=torch.nn.SiLU))    # 当前算子不支持转换
        self.features = paddle.nn.Sequential(*layers)
        self.avgpool = paddle.nn.AdaptiveAvgPool2D(1)
        self.classifier = paddle.nn.Sequential(paddle.nn.Dropout(dropout), paddle.nn.Linear())
        for m in self.modules():
            if isinstance(m, torch.nn.Conv2d):    # 当前算子不支持转换
                paddle.nn.initializer.KaimingNormal()
                if m.bias is not None:
                    paddle.nn.initializer.Constant()
            elif isinstance(m, (torch.nn.BatchNorm2d, torch.nn.GroupNorm)):    # 当前算子不支持转换
                paddle.nn.initializer.Constant()
                paddle.nn.initializer.Constant()
            elif isinstance(m, torch.nn.Linear):    # 当前算子不支持转换
                init_range = 1.0 / math.sqrt(m.out_features)
                paddle.nn.initializer.Uniform(init_range)
                paddle.nn.initializer.Constant()
...
def efficientnet_b5(*, weights: Optional[EfficientNet_B5_Weights]=None, progress: bool=True, **kwargs: Any) -> EfficientNet:
    ...
    return _efficientnet(inverted_residual_setting, kwargs.pop('dropout', 0.4), last_channel, weights, progress, norm_layer=partial(torch.nn.BatchNorm2d, eps=0.001, momentum=0.01), **kwargs)    # 当前算子不支持转换
...
def efficientnet_b6(*, weights: Optional[EfficientNet_B6_Weights]=None, progress: bool=True, **kwargs: Any) -> EfficientNet:
    ...
    return _efficientnet(inverted_residual_setting, kwargs.pop('dropout', 0.5), last_channel, weights, progress, norm_layer=partial(torch.nn.BatchNorm2d, eps=0.001, momentum=0.01), **kwargs)    # 当前算子不支持转换
...
def efficientnet_v2_s(*, weights: Optional[EfficientNet_V2_S_Weights]=None, progress: bool=True, **kwargs: Any) -> EfficientNet:
    ...
    return _efficientnet(inverted_residual_setting, kwargs.pop('dropout', 0.2), last_channel, weights, progress, norm_layer=partial(torch.nn.BatchNorm2d, eps=0.001), **kwargs)    # 当前算子不支持转换
...
def efficientnet_v2_m(*, weights: Optional[EfficientNet_V2_M_Weights]=None, progress: bool=True, **kwargs: Any) -> EfficientNet:
    ...
    return _efficientnet(inverted_residual_setting, kwargs.pop('dropout', 0.3), last_channel, weights, progress, norm_layer=partial(torch.nn.BatchNorm2d, eps=0.001), **kwargs)    # 当前算子不支持转换
...
def efficientnet_v2_l(*, weights: Optional[EfficientNet_V2_L_Weights]=None, progress: bool=True, **kwargs: Any) -> EfficientNet:
    ...
    return _efficientnet(inverted_residual_setting, kwargs.pop('dropout', 0.4), last_channel, weights, progress, norm_layer=partial(torch.nn.BatchNorm2d, eps=0.001), **kwargs)    # 当前算子不支持转换