...
class InvertedResidual(paddle.nn.Layer):
    ...
    def __init__(self, cnf: InvertedResidualConfig, norm_layer: Callable[..., torch.nn.Module], se_layer: Callable[..., torch.nn.Module]=partial(SElayer, scale_activation=torch.nn.Hardsigmoid)):    # 当前算子不支持转换
        super().__init__()
        if not 1 <= cnf.stride <= 2:
            raise ValueError('illegal stride value')
        self.use_res_connect = cnf.stride == 1 and cnf.input_channels == cnf.out_channels
        layers: List[paddle.nn.Layer] = []
        activation_layer = paddle.nn.Hardswish if cnf.use_hs else paddle.nn.ReLU
...
class MobileNetV3(paddle.nn.Layer):
    ...
    def __init__(self, inverted_residual_setting: List[InvertedResidualConfig], last_channel: int, num_classes: int=1000, block: Optional[Callable[..., torch.nn.Module]]=None, norm_layer: Optional[Callable[..., torch.nn.Module]]=None, dropout: float=0.2, **kwargs: Any) -> None:    # 当前算子不支持转换
        """
        ...
        Args:
            ...
            block (Optional[Callable[..., torch.nn.Module]]): Module specifying inverted residual building block for mobilenet
        ...
        if norm_layer is None:
            norm_layer = partial(torch.nn.BatchNorm2d, eps=0.001, momentum=0.01)    # 当前算子不支持转换
        layers: List[paddle.nn.Layer] = []
        firstconv_output_channels = inverted_residual_setting[0].input_channels
        layers.append(Conv2dNormActivation(3, firstconv_output_channels, kernel_size=3, stride=2, norm_layer=norm_layer, activation_layer=torch.nn.Hardswish))    # 当前算子不支持转换
        for cnf in inverted_residual_setting:
            layers.append(block(cnf, norm_layer))
        lastconv_input_channels = inverted_residual_setting[-1].out_channels
        lastconv_output_channels = 6 * lastconv_input_channels
        layers.append(Conv2dNormActivation(lastconv_input_channels, lastconv_output_channels, kernel_size=1, norm_layer=norm_layer, activation_layer=torch.nn.Hardswish))    # 当前算子不支持转换
        self.features = paddle.nn.Sequential(*layers)
        self.avgpool = paddle.nn.AdaptiveAvgPool2D(1)
        self.classifier = paddle.nn.Sequential(paddle.nn.Linear(), paddle.nn.Hardswish(), paddle.nn.Dropout(dropout), paddle.nn.Linear())
        for m in self.modules():
            if isinstance(m, torch.nn.Conv2d):    # 当前算子不支持转换
                ...
                paddle.nn.initializer.Normal(0.01)
                paddle.nn.initializer.Constant()
