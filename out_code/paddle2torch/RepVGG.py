...
class ConvBN(torch.nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding, groups=1):
        ...
        self.conv = torch.nn.LazyConv2d(groups, padding, out_channels, stride, kernel_size=kernel_size)
...
class RepVGGBlock(torch.nn.Module):
    ...
    def rep(self):
        if not hasattr(self, 'rbr_reparam'):
            self.rbr_reparam = torch.nn.LazyConv2d(self.groups, self.padding, self.out_channels, self.stride, self.dilation, self.kernel_size, self.padding_mode)
    ...
    def _fuse_bn_tensor(self, branch):
        ...
        if isinstance(branch, ConvBN):
            ...
            assert isinstance(branch, paddle.nn.BatchNorm2D)    # 当前算子不支持转换
            if not hasattr(self, 'id_tensor'):
                input_dim = self.in_channels // self.groups
                kernel_value = np.zeros((self.in_channels, input_dim, 3, 3), dtype=np.float32)
                for i in range(self.in_channels):
                    kernel_value[i, i % input_dim, 1, 1] = 1
                self.id_tensor = torch.Tensor.new_tensor(requires_grad=kernel_value)
